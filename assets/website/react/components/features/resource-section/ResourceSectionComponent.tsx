...
export function ResourceSectionComponent(props: {
  title?: string;
  content?: string;
  thumbnails?: string[];
}) {
    ...
    <div className="flex">
      <aside className="space-y-6 xl:basis-1/2 content-center">
        <Fade duration={1000} triggerOnce cascade>
          ...
          <p className="font-montserrat-sans text-white w-[75%]">
            <HTMLMarkupRender rawContent={props.content ?? ""} />
          </p>
        </Fade>
      </aside>
      <aside className="xl:basis-1/2 relative">
        <figure className="w-1/2 aspect-square absolute z-10 rounded-sm overflow-hidden  shadow-[0px_10px_30px_rgba(252,250,254,0.1)]">
        ...
        <figure className="w-[62.5%] top-[55%] left-1/4 z-0 aspect-video absolute rounded-sm overflow-hidden   shadow-[0px_10px_30px_rgba(252,250,254,0.1)]">
